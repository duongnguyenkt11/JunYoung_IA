...
public class DatabaseOperations {
    ...
    public void deleteFromDB(String tableName, String columnName1, String value1,String columnName2, String value2) throws Exception {
        deleteFromDBAsyncCondition(tableName, columnName1, value1, columnName2, value2 );
    }
    ...
    private void deleteFromDBAsyncCondition(String tableName, String columnName1,
                                            String value1, String columnName2, String value2) throws Exception {
        DatabaseAdapter dbAdapter = new DatabaseAdapter(mContext, mDatabaseName);
        dbAdapter.createDatabase();
        dbAdapter.open();
        String query = "DELETE FROM " + tableName + " " +
        ...
        dbAdapter.close();
        Log.e("DATABASE DELETE", query);
        if (tableName == BOOKMARK_TABLE){
            EventBus.getDefault().post(new MessageEvent("DELETE_BOOKMARK_TO_DB_SUCCESS", true));
        }
    }
    ...
    public class DbOperationsAsync extends AsyncTask<String, Integer, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            if (mOperation == "save" && mTableName == USER_TABLE){
                EventBus.getDefault().post(new MessageEvent("SAVE_USER_TO_DB_SUCCESS", true));
            }
            if (mOperation == "save" && mTableName == BOOKMARK_TABLE){
                EventBus.getDefault().post(new MessageEvent("SAVE_BOOKMARK_TO_DB_SUCCESS", true));
            }
        }
    }
}
